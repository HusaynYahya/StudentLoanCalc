...
(function () {
  ...
  /* ---- what the fees would have grown to, at any rate ---------------------
  ...
  function renderSavingsCurve(sim) {
    ...
    var base = at(0);
    if (!base || !(base.upfrontPaid > 0)) {
      host.innerHTML = "";
      $("saveKey").innerHTML = "";
      $("saveNote").textContent = "Nothing is borrowed on this profile, so there is no up-front cost to have saved instead.";
      return;
    }

    var paidIn = base.upfrontPaid;
    var pts = [];
    for (var i = 0; i <= 50; i++) {
      var s = (SAVE_MAX * i) / 50;
      pts.push({ s: s, v: at(s).fvUpfront });
    }
    ...
    var hereV = at(here).fvUpfront;
    var gain = hereV - paidIn;
    var live = saveHover != null;

    host.innerHTML =
      '<svg viewBox="0 0 ' + W + " " + H + '" role="img" ' +
      'aria-label="What the fees would have grown to, by savings rate">' + grid +
    ...
    $("saveNote").innerHTML =
      "The <b>" + gbp(paidIn) + "</b> of fees and living costs you would have to find in cash if you " +
      "did not borrow \u2014 paid into a savings account instead, as each instalment falls due, and left " +
      "until " + E.taxYearLabel(base.endYear - 1) + ". The flat line is the cash itself; everything above it is growth.";

    $("saveKey").innerHTML =
      '<i style="color:var(--good)">The pot, at ' + pct(here) + (live ? " (hovering)" : " as set") +
        " \u2014 " + gbp(hereV) + "</i>" +
      '<i style="color:var(--ink-3)">The cash you would hand over \u2014 ' + gbp(paidIn) + "</i>" +
